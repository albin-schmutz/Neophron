...
static void test_exec(void)
{
	...
	LOG_MSG(LOG_SYS_TEST, "\nsys test_exec");

	i = SYS_INFO_OS;
	sys_call(SYS_CALL_GET_INFO, &i, &i);

	if (i == 1) {
		strcpy(buf.data, "tmp_system_test.exe gugus \"Hello !\"   -123");
	} else {
...
{
	LOG_INIT();
	//LOG_ON_OFF(LOG_SYS_INIT, 1);
	//LOG_ON_OFF(LOG_SYS_GET_INFO, 1);
	//LOG_ON_OFF(LOG_SYS_BUF_RESULT, 1);
	//LOG_ON_OFF(LOG_SYS_GET_ARG, 1);
	//LOG_ON_OFF(LOG_SYS_GET_ENV, 1);
	//LOG_ON_OFF(LOG_SYS_READ_FILE, 1);
	//LOG_ON_OFF(LOG_SYS_WRITE_FILE, 1);
	//LOG_ON_OFF(LOG_SYS_TEST, 1);
