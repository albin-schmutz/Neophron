...
static SHORTINT entries[MAX_ENTRIES];
static INTEGER next_free;
...
void af_create(ALLOC_FREE af, INTEGER size)
{
	assert(af);
	assert(size > 0);
	af->start = next_free;
...
void af_alloc(ALLOC_FREE af, SHORTINT *entry)
{
	assert(af);
	assert(entry);
	if (af->free < af->size) {
...
void af_free(ALLOC_FREE af, SHORTINT entry)
{
	assert(af);
	assert(entry >= 0);
