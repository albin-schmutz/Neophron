...
static struct HEADER *last_module;
...
void run_align(int *addr, int size)
{
	if (*addr <= 0) *addr -= size;
	int fix = *addr % size;
	if (fix) {
...
void run_create_module(int64_t *prog_addr, char *name, int var_size)
{
	assert(var_size <= 0);
	...
	header->prog_addr = (void*)free_mem;
	*prog_addr = free_mem;

	LOG_MSG(LOG_RUN_CREATE_MODULE,
		"run_create_module prog_addr = %016llx name = %s var_size = %i",
		*prog_addr, name, var_size);
}
...
void run_exec_module(int prog_size, int entry_point)
{
	...
	LOG_MSG(LOG_RUN_EXEC_MODULE,
		"run_exec_module prog_size = %i entry_point = %i",
		prog_size, entry_point);
	...
	if (last_module->var_size == 0 && prog_size == 0) {
		...
		if (prog_size) {
			run_align(&last_module->prog_size, WORD_SIZE);
			reg[PC] = free_mem;
...
static int read_prog(FILE *file, struct HEADER *header)
{
	...
	run_create_module(&pc, header->modulename, header->var_size);
...
static void write_prog(FILE *file, void *pc, int prog_size)
{
	struct OPCODE opc;
	void *end = pc + prog_size;
	while (pc < end) {
		opc.instr = mem32(pc);
