...
void cpu_execute(int entry_point, modulename_provider find_modulename)
{
	...
	void *base = (void*)reg[PC];
	reg[PC] = (long long)(base + entry_point);

	LOG_MSG(LOG_CPU_EXECUTE, "cpu_execute base = %p entry_point = %p", base, (void*)reg[PC]);
	...
	while (reg[PC]) {
		...
		case POP:
			...
			OC_LOG_OC_A_B_IMM_2(pc, &opc, reg[opc.b], reg[opc.a]);
			reg[opc.b] += opc.imm;
			break;
		case PSH:
			OC_LOG_OC_A_B_IMM_2(pc, &opc, reg[opc.b], reg[opc.a]);
		...
		case MOVI:
			reg[opc.a] = opc.imm;
			OC_LOG_OC_A_IMM(pc, &opc, reg[opc.a]);
			break;
		case MULI:
			reg[opc.a] = reg[opc.b] * opc.imm;
			OC_LOG_OC_A_B_IMM(pc, &opc, reg[opc.a]);
			break;
		case DIVI:
			reg[opc.a] = reg[opc.b] / opc.imm;
			OC_LOG_OC_A_B_IMM(pc, &opc, reg[opc.a]);
		...
		case LDB:
			...
			OC_LOG_OC_A_B_IMM_2(pc, &opc, reg[opc.b] + opc.imm, reg[opc.a]);
			break;
		case LDW:
		...
		case MUL + 0:
			...
			OC_LOG_OC_A_B_C(pc, &opc, 0, reg[0]);
			break;
		case MUL + 1:
			reg[1] = reg[opc.a] * reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 1, reg[1]);
			break;
		case MUL + 2:
			reg[2] = reg[opc.a] * reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 2, reg[2]);
			break;
		case MUL + 3:
			reg[3] = reg[opc.a] * reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 3, reg[3]);
			break;
		case MUL + 4:
			reg[4] = reg[opc.a] * reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 4, reg[4]);
			break;
		case MUL + 5:
			reg[5] = reg[opc.a] * reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 5, reg[5]);
			break;
		case MUL + 6:
			reg[6] = reg[opc.a] * reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 6, reg[6]);
			break;
		case MUL + 7:
			reg[7] = reg[opc.a] * reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 7, reg[7]);
			break;
		case MUL + 8:
			reg[8] = reg[opc.a] * reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 8, reg[8]);
			break;
		case MUL + 9:
			reg[9] = reg[opc.a] * reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 9, reg[9]);
			break;
		case MUL + 10:
			reg[10] = reg[opc.a] * reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 10, reg[10]);
			break;
		case MUL + 11:
			reg[11] = reg[opc.a] * reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 11, reg[11]);
			break;
		case MUL + 12:
			reg[12] = reg[opc.a] * reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 12, reg[12]);
			break;
		case MUL + 13:
			reg[13] = reg[opc.a] * reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 13, reg[13]);
			break;
		case MUL + 14:
			reg[14] = reg[opc.a] * reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 14, reg[14]);
			break;
		case MUL + 15:
			reg[15] = reg[opc.a] * reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 15, reg[15]);
			break;
		case DIV + 0:
			reg[0] = reg[opc.a] / reg[opc.b];
			OC_LOG_OC_A_B_C(pc, &opc, 0, reg[0]);
		...
		case MOV:
			reg[opc.a] = reg[opc.b];
			OC_LOG_OC_A_B(pc, &opc, reg[opc.a]);
			break;
		case NEG:
			reg[opc.a] = -reg[opc.b];
			OC_LOG_OC_A_B(pc, &opc, reg[opc.a]);
		...
		case BITO:
			...
			break;
		case SYS + 0:
			printf("Assertion failed in module %s line %lli\n",
				find_modulename(), reg[opc.a]);
			exit(1);
			break;
		...
		case SYS + 13: case SYS + 14: case SYS + 15: case SYS + 16:
			sys_call(opc.oc - SYS, (long long*)reg[opc.a], (void*)reg[opc.b]);
			break;
		}
	}

	LOG_MSG(LOG_CPU_EXECUTE, "cpu_execute done");
}
