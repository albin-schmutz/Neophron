...
{
	if (strcmp(scan->ident, scan->module)) {
...
static void import(SCANNER scan)
{
	...
	strcpy(new_scan.module, scan->ident);
	scan_take(scan);
	...
	m = tbl_open_module(new_scan.module);
...
static void module(SCANNER scan)
{
	...
	scan->module_id = tbl_open_module(scan->module);
	...
	var_size = 0;
	declarations(scan, &var_size);
	prog_addr = run_create_module(scan->module, var_size, 0, 0);
	gen_start(prog_addr);
