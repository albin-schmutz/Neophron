...
{
	if (strcmp(scan->ident, scan->module_name)) {
...
static void check_type(SCANNER scan, INTEGER x, INTEGER y, INTEGER strict)
{
...
static SYMBOL ident_def(SCANNER scan, INTEGER *s, INTEGER clazz)
{
...
static SYMBOL ident_use(SCANNER scan, INTEGER *s)
{
	tbl_set_marker(scan->ident);
...
static void make_gen(SCANNER scan, GEN gen, SYMBOL symbol, INTEGER m)
{
	assert(scan);
	assert(gen);
	assert(symbol);
	...
	if (gen->mode == CLASS_VAR) {
		if (symbol->level == 0) {
			...
			if (m != scan->module_id) {
				INTEGER offset =
					tbl_symbols[scan->module_id].val -
					tbl_symbols[m].val;
				gen->val -= offset;
			}
		} else {
...
static void sfunc(SCANNER scan, GEN x, INTEGER nr)
{
	struct GEN y;
	...
	if (nr == 1) { /* CHR */
		...
		if (x->mode != CLASS_TYPE) {
			scan_raise_error(scan, "Type ?");
		}
		x->mode = CLASS_CONST;
		x->val = tbl_types[x->type].size;
		x->type = tbl_long_int_type;
		scan_expect(scan, tk_paren_r);
		scan_take(scan);
	} else if (nr == 7) { /* ADDR */
		if (x->mode != CLASS_VAR) {
			scan_raise_error(scan, "Not addressable");
		}
		gen_addr(x);
		x->type = tbl_long_int_type;
		scan_expect(scan, tk_paren_r);
...
static void selector(SCANNER scan, GEN x)
{
	...
	while (scan->tk == tk_brack_l || scan->tk == tk_dot) {
		type = &tbl_types[x->type];
		if (scan->tk == tk_brack_l) {
...
{
	if (symbol->clazz == CLASS_TYPE) {
		x->mode = CLASS_TYPE;
		x->type = symbol->type;
		scan_take(scan);
	} else if (symbol->clazz == CLASS_SFUNC) {
...
static void factor(SCANNER scan, GEN x)
{
	...
	if (scan->tk == kw_true) {
		...
		x->val = 1;
		scan_take(scan);
	} else if (scan->tk == kw_false) {
		x->mode = CLASS_CONST;
		x->type = tbl_boolean_type;
		x->val = 0;
		scan_take(scan);
	} else if (scan->tk == tk_char) {
		x->mode = CLASS_CONST;
		x->type = tbl_char_type;
		x->val = scan->val;
		scan_take(scan);
	} else if (scan->tk == tk_int) {
		x->mode = CLASS_CONST;
		x->type = tbl_long_int_type;
		x->val = scan->val;
		scan_take(scan);
	} else if (scan->tk == tk_ident) {
		symbol = ident_use(scan, &s);
		factor_ident(scan, x, symbol);
	} else if (scan->tk == tk_paren_l) {
		scan_take(scan);
		expr(scan, x);
		scan_expect(scan, tk_paren_r);
		scan_take(scan);
	} else if (scan->tk == tk_tilde) {
		scan_take(scan);
		factor(scan, x);
		check_bool(scan, x);
		gen_not(x);
...
static void expr(SCANNER scan, GEN x)
{
	...
	simple_expr(scan, x);
	if (scan->tk >= tk_eq && scan->tk <= tk_geq) {
		op = scan->tk;
		scan_take(scan);
		y.exp_const = x->exp_const;
		simple_expr(scan, &y);
...
static void param(SCANNER scan, INTEGER *s, SYMBOL symbol, INTEGER *count)
{
	struct GEN x;
	BOOLEAN is_ref;
	...
	if (is_ref) {
		if (x.type == tbl_long_int_type &&
			tbl_types[symbol->type].form >= FORM_ARRAY) {
			x.type = symbol->type; /* ADDR */
			is_ref = FALSE;
...
static void call_stmt(SCANNER scan, SYMBOL proc)
{
	INTEGER par_count = proc->val3;
	INTEGER s = tbl_symbols[proc->scope].next;
	scan_take(scan);
	if (scan->tk == tk_paren_l) {
		scan_take(scan);
		if (scan->tk != tk_paren_r) {
			param(scan, &s, &tbl_symbols[s], &par_count);
			while (scan->tk == tk_comma) {
				scan_take(scan);
				param(scan, &s, &tbl_symbols[s], &par_count);
			}
			scan_expect(scan, tk_paren_r);
...
static void sproc_stmt(SCANNER scan, INTEGER nr)
{
	if (nr == 1) {
		assert_stmt(scan);
	} else if (nr == 2) {
...
static void statement(SCANNER scan)
{
	...
	} else if (scan->tk == kw_if) {
		if_stmt(scan);
	} else if (scan->tk == kw_repeat) {
		repeat_stmt(scan);
	} else if (scan->tk == kw_while) {
		while_stmt(scan);
	} else {
...
static void declarations(SCANNER scan, INTEGER *addr);

static void type_def(SCANNER scan, INTEGER *t, INTEGER *s, INTEGER clazz);

static void array_type(SCANNER scan, INTEGER *t)
{
	struct GEN x;
	TYPE type;
	INTEGER size;
...
static void record_field(SCANNER scan, INTEGER *addr)
{
	INTEGER t;
	INTEGER s;
...
static void record_type(SCANNER scan, INTEGER *t)
{
	INTEGER addr = 0;
...
static void type_def(SCANNER scan, INTEGER *t, INTEGER *s, INTEGER clazz)
{
	...
	*t = -1;
	...
	} else if (scan->tk == kw_array) {
		array_type(scan, t);
	} else if (scan->tk == kw_record) {
		record_type(scan, t);
	}
	if (*t == -1) {
		scan_raise_error(scan, "Type ?");
...
static void var_decls(SCANNER scan, INTEGER *addr)
{
	INTEGER t;
	INTEGER s;
	INTEGER size;
	...
	while (scan->tk == tk_ident) {
		...
		size = run_align(tbl_types[t].size, WORD_SIZE);
		*addr -= size;
		tbl_symbols[s].val = *addr;
		scan_expect(scan, tk_semic);
		scan_take(scan);
...
static void params(SCANNER scan, INTEGER *addr, INTEGER *count)
{
	SYMBOL symbol;
	INTEGER deref = 0;
	INTEGER t = -1;
...
static void import(SCANNER scan)
{
	...
	strcpy(new_scan.module_name, scan->ident);
	scan_take(scan);
	if (scan->tk == tk_assign) {
		scan_take(scan);
		scan_expect(scan, tk_ident);
		strcpy(new_scan.module_name, scan->ident);
		scan_take(scan);
	}
	m = tbl_open_module(new_scan.module_name);
	loaded = tbl_symbols[m].level;
	if (loaded == 0) {
		scan_raise_error(scan, "Cyclic imports");
	} else if (loaded == -1) { /* new */
		scan_init(&new_scan, scan);
		module(&new_scan);
	}
	tbl_symbols[imp].scope = tbl_symbols[m].scope;
	tbl_symbols[imp].is_public = 1;
}
...
static void module(SCANNER scan)
{
	...
	scan->module_id = tbl_open_module(scan->module_name);
	assert(tbl_symbols[scan->module_id].level == -1); /* new and not loaded */
	tbl_symbols[scan->module_id].level = 0; /* not loaded */

	scan_take(scan);
	scan_expect(scan, tk_semic);
	scan_take(scan);
	if (scan->tk == kw_import) {
		import(scan);
		while (scan->tk == tk_comma) import(scan);
		scan_expect(scan, tk_semic);
		scan_take(scan);
	}
	...
	var_size = 0;
	declarations(scan, &var_size);
	prog_addr = run_create_module(scan->module_name, var_size, 0, 0);
	gen_start(prog_addr);
	tbl_symbols[scan->module_id].val = prog_addr;
	procedures(scan);
	entry_point = -1;
	if (scan->tk == kw_begin) {
	...
	tbl_symbols[scan->module_id].level = 1; /* loaded */
	tbl_close_scope();
...
void parser(char *module_name)
{
	...
	strcpy(scan.module_name, module_name);
