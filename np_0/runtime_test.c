...
static void test_create_modules(void)
{
	LONGINT prog_addr;
	struct EMITTER emit;
	INTEGER header_size = sizeof(struct HEADER);

	prog_addr = run_create_module("tmpM1", -16, 0, 0);
	...
	prog_addr = run_create_module("tmpM2", 0, 0, 0);
	emit_init(&emit, prog_addr);
...
static void test_load_modules(void)
{
	...
	FILE *file = fopen("M2.npx0", "wb");
	assert(file);
	INTEGER n = fwrite(bytes, 1, sizeof(bytes), file);
	assert(n == sizeof(bytes));
	fclose(file);

	reg[0] = 0;
	assert(run_load_file("M2"));
	assert(reg[0] == 42);
}

int main(int argc, char *argv[])
{
	assert(sizeof(struct HEADER) == 64);
	test_align();
	sys_init(argc, argv);
	run_init();
	test_create_modules();
